...
async function ensureTable() {
  ...
  try {
    await db.sql('CREATE TABLE IF NOT EXISTS events (id INTEGER PRIMARY KEY AUTOINCREMENT, headline TEXT NOT NULL, text_content TEXT NOT NULL, start_year INTEGER NOT NULL, start_month INTEGER, start_day INTEGER, media_url TEXT, media_caption TEXT, group_name TEXT)');
  } catch (error) {
...
app.post('/api/events', express.json(), async (req, res) => {
  const { headline, text_content, start_year, start_month, start_day, media_url, media_caption, group_name } = req.body;
  
  if (!headline || !text_content || !start_year) {
    return res.status(400).json({ error: 'Headline, text_content, and start_year are required' });
  }
  ...
  try {
    ...
    const sql = `INSERT INTO events (headline, text_content, start_year, start_month, start_day, media_url, media_caption, group_name) 
